...
async function main() {
    try {
        ...
        const adminIdentity = await wallet.get('client-admin');
        if (!adminIdentity) {
            console.log('An identity for the admin user "client-admin" does not exist in the wallet');
        ...
        const adminUser = await provider.getUserContext(adminIdentity, 'client-admin');
